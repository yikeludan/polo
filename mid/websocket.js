...
global.user = ctxs1
subPro.on("subscribe", function (channel, count) {
    console.log('监听到订阅事件',channel, count)
});
//在pub的时候会触发 message事件，我们的所有业务处理基本就是靠监听它了
subPro.on("message", function (channel, message) {
    console.log('监听到发布事件')
...
app1.ws.use((ctx, next) => {
    ...
    //22 通道 2-1 23 通道1-2
    console.log("进入")
    subPro.subscribe("channel"+query.toid+","+query.id);

    ctx.websocket.on("message", (message) => {
        pubPro.publish("channel"+query.id+","+query.toid, message+","+query.toid)
    });
